...
class LitCNNTabularModel(L.LightningModule):
    "Seamless model for `Image` and `Tabular data` `v1`"
    def __init__(
            self ,
            cnn_model,
            tabular_model,
        ...
        self.curator = MulticlassAccuracy(num_classes=cnn_model.fc.out_features)
        self.loss_function = nn.CrossEntropyLoss()
        # self.auroc = torchmetrics.AUROC(task="multiclass", num_classes=2)
    ...
    def training_step(self, train_batch, batch_idx) -> STEP_OUTPUT:
            ...
            loss = self.loss_function(z_hat, y)
            accuracy = self.curator(z_hat, y)
            # rocauc = self.auroc(z_hat, y)
            metrics = {'train_loss': loss, 'train_acc': accuracy}
            self.log_dict(metrics, prog_bar=True)
            # self.log("train_auroc", rocauc, on_step=False, on_epoch=True)
            return loss
    def validation_step(self, val_batch, batch_idx) -> STEP_OUTPUT:
            ...
            # self.log("val_auroc", rocauc, on_step=False, on_epoch=True)
            return loss
...
class LitTabularModel(L.LightningModule):
    ...
    def __init__(
        ...
        self.learning_rate = learning_rate
        self.curator = MulticlassAccuracy(num_classes=out_sz)
        self.loss_function = nn.CrossEntropyLoss()
